...
namespace REST_API_TESTS.Unit_Tests.Services
{
    public class ProfileServiceTests
    {
        ...
        public async Task UpdateProfile_Should_ReturnProfile_When_AccountIdInUpdateProfileDTOMatchWithLoggedInAccountId()
        {
            ...
            var profile = ProfileTestHelper.GenerateValidProfile();
            _profileServiceHelper.Setup(service => service.GetProfileFromAccount(account, updateProfileDto.AccountId)).ReturnsAsync(profile);
        ...
        public async Task GetProfile_Should_ReturnProfile_When_ProfileWasFoundInLoggedInAccount()
        {
            ...
            var account = AccountTestHelper.GenerateValidFakeAccount();
            _accountRepository.Setup(service => service.GetAccountByIdAsync(Guid.Parse(userAccountId))).ReturnsAsync(account);
            _profileServiceHelper.Setup(service => service.GetProfileFromAccount(account, profileId)).ReturnsAsync(profile);
        ...
        public async Task GetProfile_Should_ReturnErrorMessage_When_ProfileWasNotFoundInLoggedInAccount()
        {
            ...
            var account = AccountTestHelper.GenerateValidFakeAccount();
            _accountRepository.Setup(service => service.GetAccountByIdAsync(Guid.Parse(userAccountId))).ReturnsAsync(account);
            _profileServiceHelper.Setup(service => service.GetProfileFromAccount(account, profileId)).ReturnsAsync((Profile)null);
        ...
        public async Task SearchProfiles_Should_ReturnProfiles_When_QueryingProfilesSuccesfully()
        {
            ...
            var profileSearchResponseDTO = ProfileTestHelper.GenerateValidProfileSearchResponseDTO();
            _profileServiceHelper.Setup(service => service.SearchProfiles(query)).ReturnsAsync(new List<ProfileSearchResponseDTO> { profileSearchResponseDTO });

            // Act
            var result = await _sut.SearchQuery(query);

            // Assert
            Assert.IsType<List<ProfileSearchResponseDTO>?>(result.Data);
        }
        ...
        public async Task SearchProfiles_Should_ReturnErrorMessage_When_QueryingProfilesFailed()
        {
            ...
            var profile2 = ProfileTestHelper.GenerateValidProfile();
            var profileSearchResponseDTO = ProfileTestHelper.GenerateValidProfileSearchResponseDTO();
            _profileServiceHelper.Setup(service => service.SearchProfiles(query)).ReturnsAsync((List<ProfileSearchResponseDTO>)null);
